...
class Gerenciador_de_envio:
    ...
    def upload_banco_de_dados_repair(self,nome_arquivo,diretorio_entrada):
        ...
        try:
            for index, row in data_to_upload.iterrows():
                ...
                query = f"INSERT IGNORE INTO {tabela_repair} (protocol,sn,ts,repair_state_id,repair_type_id) VALUES (%s, %s, %s, %s, %s,%s)"
    ...
    def upload_banco_de_dados(self, nome_arquivo, diretorio_entrada, passed_or_failed):
        ...
        cursor = None
        start_time = time.perf_counter()
        print("Subindo no Banco de Dados " + nome_arquivo)
        ...
        try:
            ...
            if passed_or_failed == 'passed':
                ...
                for _, row in data_to_upload.iterrows():
                    row = row.fillna('')
                    query = """
                        INSERT IGNORE INTO test_results 
                        (serial_number, timestamp, test_routine, test_ok, output_voltage)
                        VALUES (%s, %s, %s, %s, %s)
                    """
                    record = (
                        row['sn'],
                        self.date_s3_to_db(row['ts']),
                        row['test_routine'],
                        row['test_ok'],
                        row['output_voltage']
                    )
                    print(query)
                    print(f"Executing query with values: {record}")
                    cursor.execute(query, record)
                ...
                for _, row in data_to_upload.iterrows():
                    ...
                    query = """
                        INSERT IGNORE INTO test_results 
                        (serial_number, timestamp, test_routine, len, len_mis, output_voltage, test_ok)
            ...
            # Bloco: test_measurement
            possible_measurement_columns = [
                'sup_120v_v', 'sub_v_grd_120v_v', 'sub_v_inv_120v_v',
                'grd_ret_120v_v', 'srg_grd_120v_v', 'srg_inv_120v_v',
                'sup_220v_v', 'sub_v_grd_220v_v', 'sub_v_inv_220v_v',
                'grd_ret_220v_v', 'srg_grd_220v_v', 'srg_inv_220v_v',
                'ext_pow_grd_110perc_v', 'ext_pow_wng',
                'vo_inv_120v_100perc_v', 'com_grd_inv_130perc',
                'vo_inv_120v_v', 'emp_csm_grd_220v_w', 'emp_csm_grd_220v_va',
                'efc_vin_220v_vo_120v_perc', 'efc_vin_220v_vo_120v_input_w', 'efc_vin_220v_vo_120v_output_w',
                'pc_sc_vo_120v_a', 'pc_sc_vo_220v_v',
                'com_grd_inv_130perc_vin_120', 'com_grd_inv_130perc_vin_220',
                'emp_csm_grd_120v_w', 'emp_csm_grd_120v_va',
                'grd_ret_v',
                'efc_vin_120v_vo_220v_perc', 'efc_vin_120v_vo_220v_input_w', 'efc_vin_120v_vo_220v_output_w',
                'vo_inv_220v_100perc_v'
            ]

            actual_measurement_columns = [col for col in possible_measurement_columns if col in dataset.columns]
            data_to_upload = dataset[actual_measurement_columns].fillna('')

            # Construir a query dinamicamente
            placeholders = ', '.join(['%s'] * (len(actual_measurement_columns) + 1))  # +1 para id_test
            query_columns = 'id_test, ' + ', '.join(actual_measurement_columns)
            query = f"""
                INSERT IGNORE INTO test_measurements ({query_columns})
                VALUES ({placeholders})
            """

            for _, row in data_to_upload.iterrows():
                record = (test_id,) + tuple(row[col] for col in actual_measurement_columns)
                cursor.execute(query, record)
            ...
            print("Falha ao Subir no Banco de Dados " + nome_arquivo)
            error_message = str(e)
            now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            with open('error_log_fh.txt', 'a') as log_file:
                log_file.write("Error at " + now + '\n')
